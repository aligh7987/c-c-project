...
void SearchMember::on_search_clicked()
{
    ...
    if(!name.isEmpty()){
        ...
        QMessageBox::critical(this,"فیلد خالی","لطفا نام فرد را وارد کنید");
    }
}


void SearchMember::on_back_clicked()
{
    this->hide();
}


void SearchMember::on_edit_clicked()
{
...
void SearchMember::on_delete_2_clicked()
{
    QList<QTableWidgetItem*>list = ui->tableMembers->selectedItems();
    if(!list.isEmpty()){
        QString mmbrshipNum = list[0]->text();
